...
export const Login = () => {
  ...
  const loginReq = async (e) => {
    try {
      ...
      let res = await fetch("https://brand-and-beyond.herokuapp.com/login", {
        method: "POST",
        headers: {
      ...
      if(data.token){
        setCookie('token', {token:data.token,user:data.user.userName,role:data.user.role});
  ...
  return (
    <div style={{display:'flex',flexDirection:'row'}}>
      ...
      <Typography sx={{textAlign:'center', fontSize:34}}>Sign In to Brand And Beyond It</Typography>
